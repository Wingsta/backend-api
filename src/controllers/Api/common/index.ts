...
import { createRazorpayOrder, CREDIT_TYPES, ORDER_STATUS, PER_UNIT_CREDIT_COST, RAZORPAY_STATUS, RAZORPAY_TRANSCATION_GATEWAY, RAZORPAY_TRANSCATION_STATUS, roundOff, SUBSCRIPTION_GST, TRANSCATION_TYPE } from "../../../utils/constants";
...
class CommonController {
	...
	public static async createOrder(
		...
		let { plan, sms, whatsapp } = req.body as {
			plan: any;
			sms: number;
			whatsapp: number;
		};
		...
		let planAmount = (plan?.amount || 0);

		let totalAmount = (smsAmount || 0) + (whatsappAmount || 0) + (planAmount || 0);

		let gst = plan ? SUBSCRIPTION_GST : PER_UNIT_CREDIT_COST.GST;
		let gstAmount = totalAmount * gst;
		let totalAmountAfterTax = totalAmount + gstAmount;
		...
		if (razorpayData?.razorpayOrderId) {
			await new TranscationLogs({
				...
				gst,
				gstAmount,
				type: plan ? TRANSCATION_TYPE.SUBSCRIPTION : TRANSCATION_TYPE.ADDONS
			}).save();
	...
	private static async updateOrderToCompany(
		...
		if (type === TRANSCATION_TYPE.SUBSCRIPTION) {
			await Company.findByIdAndUpdate(companyId, {
				subscribedPlan: plan?.subscribedPlan,
				trial: false,
				planStartDate: moment().utcOffset('+05:30').format("YYYY-MM-DD"),
				planEndDate: moment().utcOffset('+05:30').add(365-1, "days").format('YYYY-MM-DD')
			});
		} else if (type === TRANSCATION_TYPE.ADDONS) {
			await Company.findByIdAndUpdate(companyId, {
				sms: {
					...(company.sms || {
						value: 0,
